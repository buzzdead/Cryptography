...
def start_des():
    ...
    if mode == "0":
        decrypt = True
        input_file = open("ciphertext.txt", "r")
    ...
    plaintext = input_file.readline().strip()
    input_file.close()
    ...
    iv = BitVector(hexstring="7A65B3757269A47E")  # The initial vector for CBC
    output_text = ""

    if decrypt:
        round_keys.reverse() # When decrypting it starts with the last round key naturally.
    ...
    for bit_pos in range(0, total_bits, 64):
        ...
        iv = run_des(round_keys, R, L, ip)  # Since iv is basically the same as the ciphertext in CBC.

        if decrypt and not one_block:
            last_block = (bit_pos == total_bits - 64)

            if bit_pos > 0:
                # Takes the new cipher text (the iv) and xor it with the old block. (To get the real ciphertext)
            ...
            output_text += iv.getHexStringFromBitVector()
        L.clear()
        R.clear()

    # Reorders the plaintext if the text is larger than one block and removes padding if necessary.
    if decrypt and not one_block:
